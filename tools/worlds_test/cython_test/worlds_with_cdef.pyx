...
def n_assignments(n_clubs: int, naive: bool=False) -> float:
    cdef int n_block_1, n_block_2, n_block_3, divisor
...
def assignments_yield(n_clubs: int) -> Iterator[set]:
    cdef int n_block_1, n_block_2, i, _c

    n_block_1 = n_clubs // 3 + ((n_clubs % 3) > 0)
    n_block_2 = n_clubs // 3 + ((n_clubs % 3) > 1)

    def _get_runs() -> Iterator[tuple]:
        cdef int r, n, step, limit, p_r, p_n, dupes, goods, s_r, s_n, change, sub_runs
...
def generate_worlds(n_clubs: int, n_to_yield: int) -> None:
    cdef float total, yielded
